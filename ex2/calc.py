...
def click_number(event): #文字をつなげる関数
    btn = event.widget
    num = btn["text"]
    #tkm.showinfo(f"{num}", f"{num}のボタンが押されました")
    entry.insert(tk.END, num) #練習5
    
def click_equal(event):#イコールの処理をする関数
    eqn = entry.get()
    eqn = eqn.replace("÷", "/")
    eqn = eqn.replace("×", "*")
    res = eval(eqn)
...
def click_del(event):#後ろ一文字を消去する関数
    dln = entry.get()
    dln = dln[0:-1]
    entry.delete(0, tk.END)
...
root = tk.Tk()
root.geometry("400x500") #画面の大きさを400x500にする
...
for i, num in enumerate(numbers+shousu, 1):#数字のボタンを生成
...
operators = ["×", "÷", "-", "+"]
for i, ope in enumerate(operators, 1):#+-×÷のボタンの生成
    btn = tk.Button(root, text=f"{ope}", font=("", 30), width=4, height=2)
    btn.bind("<1>", click_number)
    btn.grid(row=i, column=3)

btn_eq = tk.Button(root, text="=", font=("", 30), width=4, height=2)#=のボタンの生成
